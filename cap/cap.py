...
class StereoCamera:
    ...
    def _capture_init(self):
        ...
        with open(self.capture_configuration, 'r', encoding = 'utf-8') as f:
            ...
            if params['captureMode'] == 'camera':
                ...
                # # 配置曝光模式
                # if exp_mode == 'manual':
                #     self.captureData.set(cv2.CAP_PROP_AUTO_EXPOSURE, 1.0)    # 1.0 = 手动模式 (V4L2)
    ...
    def get_split_img(self):
        ...
        self.bgr_left = frame[:, :self.imageSize[0]].copy()
        self.bgr_right = frame[:, self.imageSize[0]:].copy()
        return True
