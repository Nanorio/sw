...
def outcome_action(results, xyz_matrix, rectify_bgr_img, camera_matrix, dist_coeffs):
    ...
    if len(results[0].boxes) > 0:
        for target in results[0].boxes:
            ...
            if 0 <= cy < xyz_matrix.shape[0] and 0 <= cx < xyz_matrix.shape[1]:
                depth_mm = xyz_matrix[cy, cx, 2]   # 单位毫米
                # 有效深度判断（排除 OpenCV 无效标记 10000mm）
                if 0 < depth_mm < 9900:
                    depth_m = depth_mm / 1000.0
                    print(f"发现目标 [{cls_name}] | 中心像素点: ({cx}, {cy}) | 真实距离: {depth_m:.2f}m")
                    valid_detections = True
                    cv2.circle(save_img, (cx, cy), 5, (0, 255, 0), -1)  # 有效目标用绿色大圆点
                else:
                    print(f"发现目标 [{cls_name}] | 中心像素点: ({cx}, {cy}) | 深度无效 (Z={depth_mm}mm)")
                    cv2.circle(save_img, (cx, cy), 3, (0, 0, 255), -1)   # 无效用红色小圆点
            else:
                print(f"坐标越界 ({cx}, {cy})")

    # ================== PnP 解算（非阻塞） ==================
    # 注意：你需要根据实际场景提供三个已知 3D 点对应的 2D 像素点
    # 这里给出示例：假设目标物体上的三个特征点可以通过其他方式自动获取
    # 下面用检测框的三个角点作为临时占位（实际应替换为你自己的特征点提取逻辑）
    if valid_detections:
        # 【TODO】请替换为你的自动特征点检测结果，这里演示用检测框的三个角点
        # 顺序必须与 OBJECT_POINTS_MM 中的顺序一致
        # 例如左下方、右下方、上方三个点
        x1, y1, x2, y2 = target.xyxy[0]   # 使用最后一个检测框（若多个目标，需自行处理）
        # 假设三个特征点为：左下角、右下角、上边中点
        auto_points = [
            (int(x1), int(y2)),            # 左下角
            (int(x2), int(y2)),            # 右下角
            (int((x1+x2)/2), int(y1))      # 上边中点
        ]
        # 调用 PnP 解算并显示非阻塞 3D 截图
        from pnp_solver import solve_and_visualize
        solve_and_visualize(auto_points, camera_matrix, dist_coeffs)
...
if __name__ == '__main__':
    ...
    while True:
        ...
        cam.cpt_disparity() # 2
        cam.cpt_xyz() # 0.05s
        outcome_action(yolo.results, cam.xyz, cam.rectify_bgr_left, cam.left_matrix, cam.left_distortion)
