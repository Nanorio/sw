...
def outcome_action(results, xyz_matrix):
    for target in results[0].boxes:
        ...
        # 3. 纯终端打印输出
        if 0 < depth < 10000:  
            print(f"发现目标 [{cls_name}] | 中心像素点: ({cx}, {cy}) | 真实距离: {depth:.2f}")
        else:
...
if __name__ == '__main__':
    ...
    while True:
        ...
        print(f"[{datetime.datetime.now().strftime('%S.%f')[:6]}]") 
        ...
        print(f"[{datetime.datetime.now().strftime('%S.%f')[:6]}]")
        cam.cpt_disparity() # 2s
        print(f"[{datetime.datetime.now().strftime('%S.%f')[:6]}]")
        cam.cpt_xyz() # 0.05s
        print(f"[{datetime.datetime.now().strftime('%S.%f')[:6]}]")
        outcome_action(yolo.results, cam.xyz)
